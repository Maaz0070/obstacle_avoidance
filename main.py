...
class LiVFH:
    def __init__(self, robot_radius, safety_distance, turn_radius, measurement_threshold, wide_opening_deg=10):
        ...
        self.mu2 = 2
        self.mu3 = 2
        self.robot_radius_safty_distance = self.robot_radius + self.safety_distance
    ...
    def enlarge_obstacle(self, distance) -> int:
        ...
        # """
        angle_to_expand_by = round(((math.atan(self.robot_radius+self.safty_distance/distance)/2)/math.pi)*180)
    ...
    def calculate_is_blocked_right(self, lidar_scan: list) -> int:
        ...
        for index in range(359, 180):
            if distances[index] < (self.turn_radius + (self.robot_radius + self.safty_distance)):
                phi_r = index
                return phi_r
        return phi_r
